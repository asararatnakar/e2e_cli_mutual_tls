...
export ORD_CLIENT_CERT=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/users/Admin@example.com/tls/client.crt
export ORD_CLIENT_KEY=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/users/Admin@example.com/tls/client.key
export PEER1_CLIENT_CERT=/etc/hyperledger/fabric/admin1/tls/client.crt
export PEER1_CLIENT_KEY=/etc/hyperledger/fabric/admin1/tls/client.key
export PEER2_CLIENT_CERT=/etc/hyperledger/fabric/admin2/tls/client.crt
export PEER2_CLIENT_KEY=/etc/hyperledger/fabric/admin2/tls/client.key
MUTUAL_TLS_ARGS="--tls --cafile $ORDERER_CA --certfile $ORD_CLIENT_CERT --keyfile $ORD_CLIENT_KEY --clientauth"
...
setGlobals () {
	...
	if [ $ORG -eq 1 ] ; then
		...
		MUTUAL_TLS_ARGS="--tls --cafile $ORDERER_CA --certfile $PEER1_CLIENT_CERT --keyfile $PEER1_CLIENT_KEY --clientauth"
		if [ $PEER -eq 0 ]; then
			CORE_PEER_ADDRESS=peer0.org1.example.com:7051
		else
			CORE_PEER_ADDRESS=peer1.org1.example.com:7051
		fi
		CORE_PEER_TLS_CLIENTKEY_FILE=/etc/hyperledger/fabric/admin1/tls/client.key
    CORE_PEER_TLS_CLIENTCERT_FILE=/etc/hyperledger/fabric/admin1/tls/client.crt
	elif [ $ORG -eq 3 ] ; then
		...
		MUTUAL_TLS_ARGS="--certfile $PEER2_CLIENT_CERT --keyfile $PEER2_CLIENT_KEY --clientauth"
		if [ $PEER -eq 0 ]; then
...
joinChannelWithRetry () {
	...
	peer channel join -b $CHANNEL_NAME.block --tls --cafile $ORDERER_CA $MUTUAL_TLS_ARGS >&log.txt
	res=$?
	cat log.txt
...
installChaincode () {
	...
	ORG=$2
	setGlobals $PEER $ORG
	peer chaincode install -n mycc -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/example02/cmd $MUTUAL_TLS_ARGS >&log.txt
	res=$?
	cat log.txt
...
chaincodeQuery () {
	...
	do
        	...
        	peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["query","a"]}' $MUTUAL_TLS_ARGS >&log.txt
